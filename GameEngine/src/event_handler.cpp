...
namespace Engine
{
	EventHandler::EventHandler(const std::shared_ptr<sf::RenderWindow>& render_window, const CustomCursor& custom_cursor, const Grid& grid, const Camera &camera) noexcept :
																						 render_window_(render_window), custom_cursor_(custom_cursor), grid_(grid), camera_(camera)
	{
